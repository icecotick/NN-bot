...
async def play(ctx, url):
    ...
    # Получаем URL аудио с YouTube
    with yt_dlp.YoutubeDL({'format': 'bestaudio', 'outtmpl': 'song.%(ext)s'}) as ydl:
        info = ydl.extract_info(url, download=False)
        url2 = info['url']
    
    # Останавливаем текущую музыку и начинаем воспроизведение
    vc.stop()
    # Указываем путь к FFmpeg
    vc.play(discord.FFmpegPCMAudio(url2, executable="ffmpeg"), after=lambda e: print('Готово'))
